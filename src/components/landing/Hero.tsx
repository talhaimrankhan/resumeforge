...
export default function Hero() {
  ...
  return (
    <section
      aria-label="Hero — Free Resume Builder"
      className="relative min-h-screen flex flex-col items-center justify-start sm:justify-center overflow-hidden bg-forge-950 pt-24 sm:pt-16 pb-16 sm:pb-0"
    >
      ...
      <div className="relative z-10 text-center max-w-5xl mx-auto px-4 sm:px-6">
        {/* Badge */}
        <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur border border-white/10 rounded-full px-4 py-1.5 mb-6 sm:mb-8">
        ...
        {/* Headline */}
        <h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold text-white leading-tight tracking-tight mb-5 sm:mb-6">
          Free Resume Builder{' '}
        ...
        {/* Stats */}
        <div className="mt-8 sm:mt-16 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 max-w-2xl mx-auto">
          {[
            ...
            <div
              key={stat.label}
              className="flex flex-col items-center gap-1 sm:gap-1.5 bg-white/5 hover:bg-white/10 backdrop-blur-sm border border-white/10 hover:border-white/20 rounded-2xl px-3 sm:px-4 py-4 sm:py-5 transition-all"
            >
              <span className={`text-xl sm:text-2xl leading-none ${stat.iconClass}`}>{stat.icon}</span>
              <span className={`text-xl sm:text-2xl font-extrabold tracking-tight ${stat.valueClass}`}>
                {stat.value}
              </span>
      ...
      {/* Scroll indicator — only visible on tall screens where it fits */}
      <div className="hidden sm:block absolute bottom-8 left-1/2 -translate-x-1/2 text-white/30 animate-bounce">
